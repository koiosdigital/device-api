...
const handleRequestRenderMessage = async (ws: WebSocketAdapter, message: RequestRender) => {
...
export const matrxMessageHandler = async (ws: WebSocketAdapter, message: KDMatrxMessage) => {
  if (message.message.case === 'requestRender') {
    await handleRequestRenderMessage(ws, message.message.value);
  } else if (message.message.case === 'requestSchedule') {
    await handleScheduleRequestMessage(ws, message.message.value);
...
export const matrxQueueHandler = async (ws: WebSocketAdapter, message: any) => {
  ...
  if (msg.type === 'render_result') {
    ...
    await handleScheduleRequestMessage(ws, message as unknown as RequestSchedule);
  }
};
